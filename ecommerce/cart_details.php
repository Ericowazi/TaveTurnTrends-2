	...
	if(isset($_SESSION['user'])){
		...
		try{
			...
			foreach($stmt as $row){
				...
				$output .= "
					<tr class='tablle'>
						<td><button type='button' data-id='".$row['cartid']."'  id='buttton' class='btn btn-danger btn-flat cart_delete'><i class='fa fa-remove'></i></button></td>
						<td><img src='".$image."' width='30px' height='30px'></td>
						<td>".$row['name']."</td>
						<td>&#36; ".number_format($row['price'], 2)."</td>
						<td id='quantityy' class='input-group'>
